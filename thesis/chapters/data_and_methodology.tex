...
To eliminate lookahead contamination, my data pipeline (\texttt{data\_}\allowbreak\texttt{pipeline.py} in \texttt{src/}) ingests official Bloomberg publication timestamps (\texttt{earnings\_}\allowbreak\texttt{announcement\_}\allowbreak\texttt{date} and \texttt{filing\_}\allowbreak\texttt{date}). On any given trading date $t$, a quarterly fundamental feature vector $\mathbf{x}_{i,t}^{\text{fund}}$ is updated to the newly reported accounting state $Q_{\tau}$ if and only if:
\begin{equation}
